    ...
    def train_particle(self):
        ...
        for index, layer in enumerate(step_weight):
            ...
            # Eq. (3.3)
                ...
                if r4 < (0.5):
                    # Eq. (3.1)
                    new_weight[index]  = layer + (
                        r1 * np.sin(r2) * abs(r3 * self.global_best_model.get_weights()[index]- layer)
                    )
                else:
                    # Eq. (3.2)
                    new_weight[index]  = layer + (
                        r1 * np.cos(r2) * abs(r3 * self.global_best_model.get_weights()[index] - layer)
                    )
...
if __name__ == "__main__":
    ...
    write_csv("FedSCA", server_evaluate_acc)
